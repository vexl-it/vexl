...
- **Do NOT add comments** unless the logic is non-obvious. No section dividers (`// --- Styled primitives ---`), no labeling JSX elements (`{/* Gradient overlay */}`, `{/* Chevron */}`), no restating what the code already says. Let the code speak for itself.
