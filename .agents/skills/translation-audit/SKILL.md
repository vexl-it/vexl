...
3. **Fan out**: spawn one translator subagent per shipped locale, in parallel — each gets its locale's error list (may be empty) with the English values; model, prompt contents, and translator rules are in `docs/translating.md`. After filling its gaps, each subagent sweeps all of its locale's files and _reports_ (never rewrites) suspicious values: strings still in English, or reading machine-glued.
