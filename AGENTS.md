...
- In endpoint tests, when asserting that a request failed with an API error response, always use `expectErrorResponse` from `packages/server-utils/src/tests/expectErrorResponse.ts`.
