...
import {UserNotClubMemberError} from '@vexl-next/rest-api/src/services/contact/contracts'
    ...
    await runPromiseInMockedEnvironment(
      Effect.gen(function* (_) {
        ...
        const challengeForUser1 = yield* _(
          addChallengeForKey(user1.mainKeyPair, user1.authHeaders)({})
        )

        const challengeForUser2 = yield* _(
          addChallengeForKey(user2.mainKeyPair, user2.authHeaders)({})
        )
  ...
  it('Should return error 400 if user want to see members of a club he is not part of', async () => {
    await runPromiseInMockedEnvironment(
      Effect.gen(function* (_) {
        ...
        expectErrorResponse(UserNotClubMemberError)(errorResponse)
      })
    )
  })
