...
const sessionCredentialsAtom = atom<UserSessionCredentials | null>((get) => {
  ...
  return UserSessionCredentials.parse({
    publicKey: session.session.sessionCredentials.publicKey,
    hash: session.session.sessionCredentials.hash,
    signature: session.session.sessionCredentials.signature,
  })
})
