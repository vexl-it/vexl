...
function DonationQrCodeOrStatus({invoiceId}: Props): JSX.Element {
  ...
  const {btcAmount, exchangeRate, fiatAmount, currency, paymentLink, status} =
  ...
  useEffect(() => {
    const fiber = Effect.runFork(
      updateSingleInvoiceStatusTypeRepeating({invoiceId})
    )

    return () => {
      Effect.runFork(Fiber.interrupt(fiber))
    }
  }, [invoiceId, updateSingleInvoiceStatusTypeRepeating])
