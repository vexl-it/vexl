...
function FiatInput({
  ...
  const singlePriceValue = useAtomValue(singlePriceValueAtom)
  const calculateSatsValueOnFiatValueChange = useSetAtom(
    calculateSatsValueOnFiatValueChangeActionAtom
  )
  const toggleCurrency = useSetAtom(toggleCurrencyActionAtom)
  ...
  return (
    <CalculatorInput
      ...
      value={singlePriceValue !== 0 ? String(singlePriceValue) : undefined}
      onChangeText={(input) => {
