...
export default function PhoneNumberHashBugMigration({
  ...
  useEffect(() => {
    void (async () => {
      ...
      const hashByApp = await hashPhoneNumber(session.session.phoneNumber)

      // Note: hashPhoneNumber returns a Promise that resolves to Either-like object
      if (
        // Fallback if we can not hash phone number. Let's not block the app in that case...
        hashByApp._tag === 'Left' ||
        // If the hash is the same. It's all good
        (hashByApp._tag === 'Right' &&
          hashByApp.right === session.session.sessionCredentials.hash)
      ) {
        console.log(
          '🫣 PhoneHashBugCheck',
          'Hash is ok, no need for migration.'
        )
        setMigrationDone(true)
        return
      }
        ...
        Effect.gen(function* (_) {
          ...
          yield* _(
            contactApi.updateBadOwnerHash({
              ...
              oldData: {
                hash: Schema.decodeSync(HashedPhoneNumberE)(
                  oldCredentials.hash
                ),
                signature: Schema.decodeSync(EcdsaSignature)(
          ...
          store.set(sessionAtom, Option.some(newSession))
