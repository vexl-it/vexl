...
export const ContactInfoE = Schema.Struct({
  name: Schema.String,
  label: Schema.OptionFromOptional(Schema.String).pipe(
  ...
  nonUniqueContactId: Schema.OptionFromOptional(NonUniqueContactIdE).pipe(
    Schema.withConstructorDefault(Effect.succeed(Option.none()))
  ),
...
export const ContactFlags = Schema.Struct({
  ...
  invalidNumber: Schema.Literals(['notTriedYet', 'valid', 'invalid']),
})
export type ContactFlags = typeof ContactFlags.Type

export const StoredContact = Schema.Struct({
  info: ContactInfoE,
  computedValues: Schema.OptionFromOptional(ContactComputedValues).pipe(
    Schema.withConstructorDefault(Effect.succeed(Option.none()))
  ),
  serverHashToClient: Schema.OptionFromOptional(
    ServerToClientHashedNumber
  ).pipe(Schema.withConstructorDefault(Effect.succeed(Option.none()))),
  flags: ContactFlags.pipe(
    Schema.withDecodingDefaultType(
      Effect.sync((): typeof ContactFlags.Type => ({
        seen: false,
        imported: false,
...
export const ContactsFilterE = Schema.Literals([
  'submitted',
  'nonSubmitted',
  'new',
  'all',
