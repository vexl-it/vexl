  ...
  registerInAppLoadingTask({
    ...
    task: (store) =>
      Effect.gen(function* (_) {
        ...
        console.log(
          `Finished refreshing offers. Successfully refreshed: ${successCount}, failed: ${failedCount}`
        )
        void showDebugNotificationIfEnabled({
          title: 'refreshing notification tokens offers',
          subtitle: 'checkNotificationTokensAndRefreshOffersActionAtom',
          body: `Finished refreshing offers. Successfully refreshed: ${successCount}, failed: ${failedCount}`,
        })
