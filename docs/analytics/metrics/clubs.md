...
- **Privacy notes**: the backend row carries `clubUUid` and an exact timestamp; the client record carries neither.
...
- **Status: planned**
- **Type**: aggregation `clubActivityMonthly` (month, no country). This section defines the record: booleans `isClubMember`, `firstClubJoin`; counter `clubJoins` (cap 10); buckets `membership` (`MembershipBucket`), `clubsActedIn`, `clubOfferCreations`, `clubRequests` (`CountBucket`).
