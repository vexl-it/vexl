...
| `updatedDay`    | `YYYY-MM-DD`                 | Day of the last local state change. Must be `>= startDay`, or `>=` its ISO week start for long-lived journeys with `updatedDayPrecision: 'week'`. Both dates may be at most one day in the future for clock skew. |
...
- `flush.ts`: `flushAnalyticsActionAtom` prunes, then uploads pending entries through `api.metrics.upsertAnalyticsState`. Journey-triggered flushes only upload journeys; lifecycle flushes include aggregations. Before each request it checks consent and reads the current outbox entry, so clearing the outbox also stops entries captured by an earlier flush. Success or any 4xx except 429 settles the entry (4xx is reported with `name` and status only); 5xx or a network error keeps it for the next flush. Flushes are serialized through a semaphore, so a step recorded while a request is in flight goes out in the following flush with the latest state instead of racing it (section 6.4).
...
- Turning it off: clear the outbox, delete all journey and aggregation ids locally, stop recording. A request already in flight may finish, but the flush starts no further requests for the cleared entries, even if analytics is turned on again. Rows already on the server expire normally.
