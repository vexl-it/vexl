...
import {ecdsaSign, ecdsaVerify} from './index'

const key = PrivateKey.import({
  key: '4y6hUGdwwsdOPkAU3gRZiTmjrc+glpYl345KmQ==',
  type: KeyFormat.RAW,
})
console.log(key.exportPrivateKey(KeyFormat.RAW))

const signature = ecdsaSign({challenge: 'ahoj', privateKey: key})
const verified = ecdsaVerify({
  challenge: 'ahoj',
  signature:
    'MD0CHQCYx8Wox5l4hTc3m5OsAccBDW/ZXkZB75Z47RyaAhwS+eym2b3ubSuaBq3zTR2hkbud5uDFO+D+AcL2',
  pubKey: key,
})
console.log(key.toLog())
