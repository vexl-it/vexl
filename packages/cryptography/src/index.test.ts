...
import {stripVersion} from './versionWrapper.js'
import {KeyFormat, PrivateKey, PublicKey} from './KeyHolder.js'
import * as VCrypto from './index'

const privateKeyFromRaw = PrivateKey.import({
  key: 'b3fQbNfTqCJVO5KUd/iKdRA/gv8qWyvXuriLTQ==',
  type: KeyFormat.RAW,
})

const privateKeyFromPem = PrivateKey.import({
  key: '',
...
describe('aes', () => {
  ...
  it('Should successfully encrypt and decrypt', () => {
    ...
    const decrypted = aesDecrypt({data: encrypted, password})
...
function ecies(key: PrivateKey): boolean {
  const textToEncrypt = crypto.randomBytes(1111).toString('hex')

  const encrypted = eciesCTREncrypt({
    publicKey: key,
