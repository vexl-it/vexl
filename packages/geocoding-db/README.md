...
1. the most specific **sub-city** boundary is the label; the covering
   **city** boundary is the "…, City" context when it is a city/town, else
   the best city/town node within 30 km — importance minus 0.005 per km,
   same country first (a municipality can be a single village — "Černilov,
   Hradec Králové"; London boroughs carry no city tag — "Brixton, London",
   while Taipei still beats the bigger New Taipei next door);
