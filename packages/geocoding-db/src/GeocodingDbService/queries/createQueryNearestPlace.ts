...
/** A sub-city label gets "…, City" from the nearest city/town within this. */
...
export const createQueryNearestPlace = Effect.gen(function* (_) {
  ...
  /**
   * The "…, City" context: the most important city/town node within 30 km,
   * preferring the label's own country — so Brixton reads "…, London" (not
   * the nearer place=city node of Westminster) and an Austrian village next
   * to Bratislava keeps an Austrian town.
   */
  ...
  > =>
    Effect.gen(function* (_) {
      ...
      const {subCity, city} = resolveBoundaryCandidates(
      ...
      if (Option.isSome(city)) {
        ...
        if (Option.isSome(node)) {
          return Option.some(
            withContext(node.value, yield* _(contextInside(city, node.value)))
          )
        }
        const label = boundaryAsSettlement(
          city.value,
