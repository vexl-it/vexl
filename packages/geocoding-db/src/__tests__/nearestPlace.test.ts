 ...
 *   Kommune (level 7, country no) [15.5-15.6, 52.6-52.7] — city via override
 *   Border part (place=neighbourhood, ingested as pl) inside Cityville
 *     [13.15-13.19, 49.15-49.19] — the country comes from Cityville
  ...
  yield* _(sql`
    ...
    VALUES
      ...
      (
        107,
        0,
        ST_MakeEnvelope (15.5, 52.6, 15.6, 52.7, 4326)
      ),
      (
        108,
        0,
...
describe('nearestPlace', () => {
  ...
  it('takes the country from the largest covering boundary, not the label', async () => {
    const place = await nearest(49.17, 13.17)
    expect(place.name).toBe('Border part')
    expect(Option.getOrThrow(place.countryCode)).toBe('cz')
    expect(Option.getOrThrow(place.cityName)).toBe('Cityville')
  })
  ...
  it('applies country-specific level roles (Norwegian kommune at level 7)', async () => {
    const place = await nearest(52.65, 15.55)
  ...
  it('scores context by importance and distance: a big city a few km away beats a small one next door', async () => {
