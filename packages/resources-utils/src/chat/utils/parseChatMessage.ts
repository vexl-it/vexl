...
const ChatMessageRequiringNewerVersionWithDefaults = Schema.Struct({
  ...
  time: Schema.optionalWith(UnixMilliseconds, {
    default: () => unixMillisecondsNow(),
  }),
  uuid: Schema.optionalWith(ChatMessageId, {
    default: () => generateChatMessageId(),
  }),
  text: Schema.optionalWith(Schema.Literal('-'), {default: () => '-'}),
})
